% \subsection{Generalized Trend Filtering}
% One interesting application of this general framework is generalized trend filtering. In trend filtering, we use a fixed filter $w_D = (-1, 2, -1)$, we can relax the problem by looking for a filter $w$ while solving trending filtering problem. 
%  \begin{eqnarray*}
% \underset{w, x}{\mbox{minimize}}& 1/\sigma^2 \|y - x\|_2^2 +\|z\|_1+R(w) ,\\
% \mbox{subject to}& w_1 =2.
%  \end{eqnarray*}
...
% We use alternating direction method of multiplier(ADMM) to solve the problem:
%  \begin{eqnarray*}
